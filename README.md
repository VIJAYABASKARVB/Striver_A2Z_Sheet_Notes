# Striver_A2Z_Sheet_Notes

## Arrays

### ✅ Easy

| # | Problem | Solution |
|---|---------|----------|
| 01 | Largest Element in an Array | [Link](./Arrays/1.Easy/01_Largest_Element/Largest_Element_in_Array.md) |
| 02 | Second Smallest & Largest Element | [Link](./Arrays/1.Easy/02_Second_Largest_Element/Second_Largest_Element.md) |
| 03 | Check if Array is Sorted | [Link](./Arrays/1.Easy/03_Check_if_the_Array_is_Sorted_II/Check_if_an_Array_is_Sorted.md) |
| 04 | Remove Duplicates from Sorted Array | [Link](./Arrays/1.Easy/04_Remove_Duplicates_from_Sorted_Array/Remove_Duplicates_from_Sorted_Array.md) |
| 05 | Left Rotate Array by One | [Link](./Arrays/1.Easy/05_Left_Rotate_Array_by_One/Left_Rotate_Array_by_one.md) |
| 06 | Rotate Array by K Places | [Link](./Arrays/1.Easy/06_Left_Rotate_Array_by_K_Places/Left_Rotate_Array_by_K_Places.md) |
| 07 | Move Zeros to End | [Link](./Arrays/1.Easy/07_Move_Zeros_to_End/Move_Zeros_to_End.md) |
| 08 | Linear Search | [Link](./Arrays/1.Easy/08_Linear_Search/Linear_Seach.md) |
| 09 | Union of Two Sorted Arrays | ⏳ |
| 10 | Find Missing Number | [Link](./Arrays/1.Easy/10_Find_Missing_Number/Find_Missing_Number.md) |
| 11 | Maximum Consecutive Ones | [Link](./Arrays/1.Easy/11_Maximum_Consecutive_Ones/Maximum_Consecutive_Ones.md) |
| 12 | Number that Appears Once | [Link](./Arrays/1.Easy/12_Find_the_Number_that_Appears_Once_and_Other_Numbers_Twice/Find_the_Number_that_Appears_Once_and_Other_Numbers_Twice.md) |
| 13 | Longest Subarray with Sum K (Positive) | [Link](./Arrays/1.Easy/13_Longest_Subarray_with_Given_Sum_Positive/Longest_Subarray_with_Sum_K.md) |
| 14 | Longest Subarray with Zero Sum | [Link](./Arrays/1.Easy/14_Longest_Subarray_with_Sum_K/Longest_Subarray_with_Zero_Sum.md) |

### 🔶 Medium

| # | Problem | Solution |
|---|---------|----------|
| 01 | Two Sum | ⏳ |
| 02 | Sort Colors (0s, 1s, 2s) | ⏳ |
| 03 | Majority Element I | ⏳ |
| 04 | Kadane's Algorithm | ⏳ |
| 05 | Print Maximum Subarray Sum | ⏳ |
| 06 | Stock Buy and Sell | ⏳ |
| 07 | Rearrange Array by Sign | ⏳ |
| 08 | Next Permutation | ⏳ |
| 09 | Leaders in an Array | ⏳ |
| 10 | Longest Consecutive Sequence | ⏳ |
| 11 | Set Matrix Zeroes | ⏳ |
| 12 | Rotate Matrix by 90° | ⏳ |
| 13 | Spiral Matrix | ⏳ |
| 14 | Count Subarrays with Given Sum | ⏳ |

### 🔴 Hard

| # | Problem | Solution |
|---|---------|----------|
| 01 | Pascal's Triangle I | ⏳ |
| 02 | Majority Element II | ⏳ |
| 03 | 3 Sum | ⏳ |
| 04 | 4 Sum | ⏳ |
| 05 | Largest Subarray with Sum 0 | ⏳ |
| 06 | Count Subarrays with XOR K | ⏳ |
| 07 | Merge Overlapping Intervals | ⏳ |
| 08 | Merge Sorted Arrays Without Extra Space | ⏳ |
| 09 | Find Repeating & Missing Number | ⏳ |
| 10 | Count Inversions | ⏳ |
| 11 | Reverse Pairs | ⏳ |
| 12 | Maximum Product Subarray | ⏳ |

## Stack-Queue

### 💡 Learning

| # | Problem | Solution |
|---|---------|----------|
| 01 | Implement Stack using Array | [Link](./Stack-Queue/Learning/01_Implement_Stack_Using_Arrays/Implement_Stack_Using_Arrays.md) |
| 02 | Queue Using Array (Circular) | [Link](./Stack-Queue/Learning/02_Implement_Queue_Using_Arrays/Implement_Queue_Using_Arrays.md) |
| 03 | Implement Stack using Single Queue | [Link](./Stack-Queue/Learning/03_Implement_Stack_Using_Queue/Implement_Stack_Using_Queue.md) |
| 04 | Implement Queue using Stack | [Link](./Stack-Queue/Learning/04_Implement_Queue_Using_Stack/Implement_Queue_Using_Stack.md) |
| 05 | Implement Stack using LinkedList | ⏳ |
| 06 | Implement Queue using LinkedList | ⏳ |
| 07 | Check for Balanced Parentheses | [Link](./Stack-Queue/Learning/07_Balanced_Parenthesis/Balanced_Parenthesis.md) |
| 08 | Implement Min Stack | ⏳ |

### 📊 Mono-stack

| # | Problem | Solution |
|---|---------|----------|
| 01 | Next Greater Element I | [Link](./Stack-Queue/Mono-stack/01_Next_Greater_Element/Next_Greater_Element.md) |
| 02 | Next Greater Element II | ⏳ |
| 03 | Next Smaller Element | ⏳ |
| 04 | Number of Greater Elements to the Right | ⏳ |
| 05 | Trapping Rainwater | ⏳ |
| 06 | Sum of Subarray Minimums | ⏳ |
| 07 | Asteroid Collision | ⏳ |
| 08 | Sum of Subarray Ranges | ⏳ |
| 09 | Remove K Digits | ⏳ |
| 10 | Largest Rectangle in Histogram | ⏳ |
| 11 | Maximal Rectangle | ⏳ |
